...
public class Battle {
    ...
    private void executeRound() {
        ...
        List<Fighter> orderedFighters = current.stream()
                .sorted(Comparator.comparingInt(Fighter::getInnitiative).reversed())
        ...
        for (Fighter attacker : orderedFighters) {
            ...
            Fighter attacked = null; // prevents attackers from attacking themselves
            do {
            ...
            announcer.announce(String.format(
                    "%s attacks %s and drains %s balance.%s",
                    attacker.getName(),
                    attacked.getName(),
                    balanceDrain,
                    bigDamage ? "(CRIT)":""
