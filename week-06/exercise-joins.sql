...
-- Expected: 19 Rows
select i.name, i.price_per_unit,
u.name
from item i
...
select c.first_name, c.last_name, p.project_id, i.name, pi.quantity * i.price_per_unit
from customer c 
...
select *
from category c
left outer join category cp on c.parent_category_id = cp.category_id;
